...
def test_admin_catalog_requires_admin_client() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/units")

    assert response.status_code == 401
    assert response.json() == {"detail": "admin_client_required"}


def test_admin_catalog_health() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/health", headers=ADMIN_HEADERS)
...
def test_admin_catalog_units_returns_seeded_units() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/units", headers=ADMIN_HEADERS)
...
def test_admin_catalog_price_lists_returns_default_storefront_price_list() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/price-lists", headers=ADMIN_HEADERS)
...
def test_admin_catalog_products_returns_management_view() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/products", headers=ADMIN_HEADERS)
...
def test_admin_catalog_skus_returns_units_and_storefront_prices() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/skus", headers=ADMIN_HEADERS)
...
def test_admin_catalog_sku_prices_returns_default_sku_prices() -> None:
    client = TestClient(app)

    response = client.get("/admin/catalog/sku-prices", headers=ADMIN_HEADERS)
